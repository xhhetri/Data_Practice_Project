...
def load_nga_odata_api() -> pd.DataFrame:
    ...
    state_territory_ghg.csv, but via a different extraction path (JSON API
    vs. published CSV table) and different field naming (PascalCase).

    This is deliberately loaded as a SEPARATE table, not merged into the
    other loaders -- its value is as an independent cross-check. See
    validate.py: validate_ghg_cross_source().
    """
    import json

    path = _locate("nga_odata_api", ext="json")
    with open(path) as f:
        raw = json.load(f)
...
def load_nsw_traffic_counts() -> pd.DataFrame:
    """
    Hourly vehicle counts per station, NSW only. Different grain entirely
    from every other source (station x hour, not state x year), so it is
    NOT merged into the annual master tables. Used on its own for traffic
    pattern EDA and a short-horizon forecast -- see eda.py and model.py.
    """
