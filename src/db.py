...
def load_processed_to_db(engine=None) -> dict:
    ...
    plus the pipeline_runs audit row, as a single transaction -- so a
    crash mid-load can't leave fresh tables with no matching audit row,
    or vice versa. Returns a small summary dict (also used by
...
def run() -> None:
    """CLI entry point (`python -m src.db`). Owns its engine end-to-end,
    so it disposes it once the load is done -- unlike get_engine() callers
    that keep a long-lived engine around (the API/Streamlit apps), a
    one-shot script has no reason to hold the connection pool open after
    it exits."""
    engine = get_engine()
    try:
        load_processed_to_db(engine)
    finally:
        engine.dispose()
